// ════════════════════════════════════════════════════════════════════════
//  GP_MerTest.gs — GerberaPrints · MER target + FB marginal test  v1.1
//  ------------------------------------------------------------------------
...
/** Light daily refresh: updates ONLY the trailing-MER status line (row 8). Does NOT clear the sheet or
 *  touch the config/date cells (rows 4-6), so an in-progress test keeps its dates. Wired into the daily
 *  analytics trigger; safe no-op if the tool sheet hasn't been built yet. */
function _merStatusRefresh() {
  var ss = _getSSActive(), ws = ss.getSheetByName(MER_SHEET);
  if (!ws) return;   // tool not built yet -> nothing to refresh
  var target = parseFloat(ws.getRange(4, 2).getValue()) || 2.3;
  var be = parseFloat(ws.getRange(4, 5).getValue()) || 2.0;
  var revByDay = _merRevByDay(ss), sp = _dplLoadAdSpendSplit();
  var today = new Date();
  var f = Utilities.formatDate(new Date(today.getTime() - 14 * 864e5), DPL.VN_TZ, 'yyyy-MM-dd');
  var t = Utilities.formatDate(new Date(today.getTime() - 864e5), DPL.VN_TZ, 'yyyy-MM-dd');
  var cur = _merWindow(revByDay, sp, f, t);
  var bg = cur.mer < be ? '#FEE2E2' : (cur.mer < target ? '#FEF3C7' : '#DCFCE7');
  var tag = cur.mer < be ? '  \u26A0 BELOW breakeven' : (cur.mer < target ? '  \u2014 thin (below target)' : '  \u2705 above target');
  ws.getRange(8, 1).setValue('\uD83D\uDCC8 CURRENT (trailing 14d): Store Rev/day $' + Math.round(cur.revD).toLocaleString()
    + '  \u00b7  Ad/day $' + Math.round(cur.adD).toLocaleString() + '  \u00b7  Blended MER ' + cur.mer.toFixed(2) + 'x' + tag);
  ws.getRange(8, 1, 1, 6).setBackground(bg);
}
